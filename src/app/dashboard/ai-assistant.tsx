...
export default function AiAssistant() {
  ...
  const handleAskAI = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setResponse(data.error || `Server error (${res.status}): Failed to analyze document.`)
      } else {
        // Fallback checks for common API response keys
        setResponse(
          data.answer || 
          data.text || 
          data.result || 
          data.response || 
          data.error || 
          'No answer generated by the assistant.'
        )
      }
    } catch (err: any) {
      console.error('AI Processing Error:', err)
      setResponse(`An error occurred while communicating with the server: ${err.message || 'Unknown error'}`)
    } finally {
      setLoading(false)
    }
  }
                ...
                <div className="text-slate-300 text-sm leading-relaxed space-y-3">
                  <ReactMarkdown
                    components={{
                      ...
                      h3: ({ children }) => <h3 className="text-sm font-bold text-indigo-300 mt-3 mb-1">{children}</h3>,
                      ...
                      code: ({ children }) => <code className="bg-slate-950 px-1.5 py-0.5 rounded text-indigo-300 text-xs font-mono">{children}</code>,
